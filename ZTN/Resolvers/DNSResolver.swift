...
public class DNSResolver {
    ...
    public func get(_ ip: IPAddress) -> String? {
        return queue.sync {
            self.Bindings[ip]?.url
        }
    }

    public func update(_ response: DNSResponse) {
        queue.sync { [weak self] in
            ...
            response.cnames.forEach {
                self.update(url: $0.url, cName: $0.cname)
            }
    ...
    private func update(question: String) {
    ...
    private func update(url: String, ip: IPAddress) {
    ...
    private func update(url: String, cName: String) {
